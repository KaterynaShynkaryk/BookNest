from django.urls import path

from .views import (
    book_create,
    book_create_manual,
    book_create_search,
    book_delete,
    book_detail,
    book_list,
    book_note_create,
    book_toggle_favorite,
    book_update,
    note_delete,
    note_list,
    note_toggle_favorite,
    note_update,
    logout_view,
    register,
    shelf_create,
    shelf_detail,
    shelf_books,
    shelf_book_remove,
    shelf_delete,
    shelf_list,
    shelf_update,
    statistics,
    test_page,
    UkrainianLoginView,
)

urlpatterns = [
    path("", book_list, name="book_list"),
    path("books/", book_list, name="book_list"),
    path("books/add/", book_create, name="book_create"),
    path("books/add/search/", book_create_search, name="book_create_search"),
    path("books/add/manual/", book_create_manual, name="book_create_manual"),
    path("books/<int:pk>/", book_detail, name="book_detail"),
    path("books/<int:pk>/notes/add/", book_note_create, name="book_note_create"),
    path("books/<int:pk>/favorite/", book_toggle_favorite, name="book_toggle_favorite"),
    path("books/<int:pk>/edit/", book_update, name="book_update"),
    path("books/<int:pk>/delete/", book_delete, name="book_delete"),
    path("notes/", note_list, name="note_list"),
    path("statistics/", statistics, name="statistics"),
    path("shelves/", shelf_list, name="shelf_list"),
    path("shelves/add/", shelf_create, name="shelf_create"),
    path("shelves/<int:pk>/", shelf_detail, name="shelf_detail"),
    path("shelves/<int:pk>/books/", shelf_books, name="shelf_books"),
    path("shelves/<int:pk>/books/<int:book_pk>/remove/", shelf_book_remove, name="shelf_book_remove"),
    path("shelves/<int:pk>/edit/", shelf_update, name="shelf_update"),
    path("shelves/<int:pk>/delete/", shelf_delete, name="shelf_delete"),
    path("notes/<int:pk>/favorite/", note_toggle_favorite, name="note_toggle_favorite"),
    path("notes/<int:pk>/edit/", note_update, name="note_update"),
    path("notes/<int:pk>/delete/", note_delete, name="note_delete"),
    path("test/", test_page, name="test_page"),
    path("register/", register, name="register"),
    path("auth/register/", register, name="auth_register"),
    path("auth/login/", UkrainianLoginView.as_view(), name="login"),
    path("auth/logout/", logout_view, name="logout"),
]